...
char * fgets(char *dst, int max, int fd)
{
	...
	if(fd < 0)
	    printf("Cannot fgets. File does not exist or has not been opened\n");
	...
	while(1)
	{
	    ...
	    *dst++ = c;
	    ...
	    // We have copied max chars.
	    if((count + 2) == max) // Leave space for newline and null termination.
	    {
	        break;
	    } 
	}
